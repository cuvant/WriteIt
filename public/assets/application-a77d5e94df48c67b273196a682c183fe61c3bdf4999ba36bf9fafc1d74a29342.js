...
jQuery(document).ready(function($) {
    ...
    if ($.isFunction($.fn.masonry)) {
        ...
        $mElement.each(function(index, elem) {
            ...
            $(elem).on('turbolinks:load', function() {
                loaded++;
                if (count == loaded) {
                    $('.js-masonry').masonry('layout');
                }
...
(function() {
  App.appearance = App.cable.subscriptions.create("AppearanceChannel", {
    ...
    received: function(data) {
      switch (data.status) {
        case "online":
          return this.set_online(data.user);
        case "offline":
          return this.set_offline(data.user);
      }
    },
    set_online: function(user) {
      var online_ul;
      if (user !== null && $("#online_user_" + user.user_id).length === 0) {
        ...
        return online_ul.append(user.content);
      }
    },
    set_offline: function(user) {
      var online_ul;
      if (user !== null && $("#online_user_" + user.user_id).length > 0) {
        online_ul = $(".online-users");
        ...
        if (online_ul.children('li').length === 0) {
          ...
          return $("#online-title").removeClass("title-online").addClass("title-offline");
        }
      }
    }
  });
...
(function() {
  window.writeit = window.writeit || {};

  (function(micropost_channel, $) {
    return micropost_channel.create_micropost_subscription = function(micropost_id) {
      if (!App["micropost-" + micropost_id]) {
        return App["micropost-" + micropost_id] = App.cable.subscriptions.create({
          channel: "MicropostChannel",
          micropost_id: micropost_id
        }, {
          connected: function() {
            return this.micropost_id = micropost_id;
          },
          disconnected: function() {},
          received: function(data) {
            if (data["user_id"] !== "" && data["user_id"] !== currentUser) {
              return this.add_content(data);
            }
          },
          add_content: function(data) {
            switch (data.notice_type) {
              case "like":
                this.notify_like(data.like);
                break;
              case "comment":
                this.notify_comment(data.comment, data.remove);
                break;
              case "commenting":
                this.notify_commenting(data.commenting);
            }
          },
          someone_is_writing: function(micropost_id) {
            return this.perform("someone_is_writing", {
              micropost_id: micropost_id
            });
          },
          notify_like: function(content) {
            var likes;
            if (likes = $("#likes_" + content.micropost_id)) {
              return likes.html(content.partial);
            }
          },
          notify_comment: function(content, should_remove) {
            var comments;
            if (comments = $("#comments_" + content.micropost_id)) {
              if (should_remove === "false") {
                return $("#micropost_" + content.micropost_id + "_comment_" + content.partial).remove();
              } else {
                comments.append(content.partial);
                return $("#writing_comment_" + content.micropost_id).html("");
              }
            }
          },
          notify_commenting: function(micropost_id) {
            var myTimeout;
            $("#writing_comment_" + micropost_id).html("Someone is commenting...");
            if (myTimeout === null) {
              return myTimeout = setTimeout((function() {
                return $("#writing_comment_" + micropost_id).html("");
              }), 10000);
            }
          }
        });
  ...
  })(window.writeit.micropost_channel = window.writeit.micropost_channel || {}, jQuery);
...
(function() {
  App.newest_micropost = App.cable.subscriptions.create("NewestMicropostChannel", {
    ...
    received: function(data) {
      if (data.step === "first") {
        return $("#my-microposts").prepend(data.micropost);
      }
    }
  });
  ...
  App.notifications = App.cable.subscriptions.create("NotificationsChannel", {
    ...
    received: function(data) {
      ...
      if (data.notice_type === "follow") {
        this.notify_follow(data.follow);
      }
    ...
    update_counter: function(counter) {
      ...
      if (notifications = $("#notifications-counter")) {
        ...
        if (counter > 0) {
          return notifications.css('color', '#FFA5A5');
        } else {
          return notifications.css('color', 'white');
        }
      }
    },
    notify_follow: function(content) {
      ...
      if ($("#current_user_followers_count_" + content.user_id).length !== 0) {
        return $("#current_user_followers_count_" + content.user_id).html(this.text_displayed(content.followers_count));
      }
    },
    text_displayed: function(count) {
      if (count !== 1) {
...
(function() {
  ...
  (function(microposts, $) {
    microposts.show_map = function() {
      $('.show_map').click(function(e) {
        e.preventDefault();
        $(this).parent().parent().find('.map').toggle();
      });
    };
    return microposts.use_mentioner = function(user_id) {
      $(".comment_content").on("input", function() {
        var $text, micropost_id;
        $text = $(this).val();
        micropost_id = $(this).attr('id').split('_').pop();
        App["micropost-" + micropost_id].someone_is_writing({
          micropost_id: micropost_id
        });
        if ($text.substr($text.length - 2) === "@" || $text.substr($text.length - 2) === " @") {
          $.ajax({
            ...
            success: function(data) {
              data = data;
              return $("#comment_content_" + micropost_id).atwho({
                at: "@",
                'data': data
              });
            }
          });
          return console.log("users/" + user_id + "/get_following");
        }
      });
    };
  })(window.writeit.microposts = window.writeit.microposts || {}, jQuery);
...
(function() {
  ...
  (function(notifications, $) {
    notifications.remove_after_close = function() {
      return $('#notifications-dropdown').on('hide.bs.dropdown', function() {
        return $(".dropdown-menu.notifications .li-notification").remove();
      });
    };
    return notifications.set_counter_color = function() {
      if (parseInt($("#notifications-counter").html()) > 0) {
        return $("#notifications-counter").css('color', '#FFA5A5');
      } else {
        return $("#notifications-counter").css('color', 'white');
      }
    };
  })(window.writeit.notifications = window.writeit.notifications || {}, jQuery);
...
(function() {
  ...
  })(window.writeit.scrolling = window.writeit.scrolling || {}, jQuery);
...
(function() {
  ...
  })(window.writeit.shared = window.writeit.shared || {}, jQuery);
